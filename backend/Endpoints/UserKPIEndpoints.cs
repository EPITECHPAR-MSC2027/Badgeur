...
namespace badgeur_backend.Endpoints
{
    public static class UserKPIEndpoints
    {
        public static void MapUserKPIEndpoints(this IEndpointRouteBuilder app)
        {
            ...
            group.MapGet("/{userId:long}", GetUserKPIs)
                .WithDescription("Retrieve KPIs for a specific user. Users can only access their own KPIs. " +
                "Always calculates KPIs upon being called. If there is already a database entry for a User, it will update it. Otherwise it will create it.");
        }

        public static async Task<IResult> GetMyKPIs(
            UserKPIService userKPIService,
            BadgeLogEventService badgeLogEventService,
            HttpContext context)
        {
            ...
            // Using > instead of >= with -14 ensures we get exactly 14 days of data
        ...
        public static async Task<IResult> GetUserKPIs(
            long userId,
            UserKPIService userKPIService,
            HttpContext context)
        {
            ...
            if (userKPIs == null)
            {
                return Results.NotFound("Failed to calcuate KPIs. Contact an administrator.");
            }
